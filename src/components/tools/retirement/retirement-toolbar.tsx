...
export interface RetirementToolbarProps {
  currency: string;
...
export function RetirementToolbar({
  currency,
  onCurrencyChange,
  onSeed,
  onReset,
  onExport,
  onImport,
}: RetirementToolbarProps) {
  const t = useTranslations("retirement.toolbar");

  return (
    <div className="flex flex-wrap items-end gap-3">
      <div className="w-28">
        <label
          htmlFor="retirement-currency"
          className="mb-1.5 block text-sm font-medium text-muted-foreground"
        >
          {t("currency")}
        </label>
        <select
          id="retirement-currency"
          value={currency}
